RUST_TARGET := kernel/x86_64-kacos.json
LINKER_SCRIPT := linker.ld
KERNEL := kernel/target/x86_64-kacos/release/kacos
ISO := kacos.iso
BUILD_DIR := build
...
CARGO_BUILD_FLAGS := -Zbuild-std=core,compiler_builtins -Zbuild-std-features=compiler-builtins-mem -Zjson-target-spec --target x86_64-kacos.json --release
